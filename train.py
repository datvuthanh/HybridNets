...
def get_args():
    parser = argparse.ArgumentParser(
        "HybridNets: End-to-End Perception Network - DatVu"
    )
    parser.add_argument(
        "-p",
        "--project",
    ...
    parser.add_argument(
        "-n", "--num_workers", type=int, default=8, help="Num_workers of dataloader"
    )
    parser.add_argument(
        "-b",
        "--batch_size",
        type=int,
        default=12,
        help="Number of images per batch among all devices",
    )
    parser.add_argument(
        "--freeze_backbone",
        type=boolean_string,
    ...
    parser.add_argument(
        "--freeze_det", type=boolean_string, default=False, help="Freeze detection head"
    )
    parser.add_argument(
        "--freeze_seg",
        type=boolean_string,
    ...
    parser.add_argument(
        "--conf_thres", type=float, default=0.001, help="Confidence threshold in NMS"
    )
    parser.add_argument(
        "--iou_thres", type=float, default=0.6, help="IoU threshold in NMS"
    )
...
def train(opt):
    ...
    seg_mode = (
        ...
        else MULTICLASS_MODE
        if len(params.seg_list) > 1
        else BINARY_MODE
    )
    ...
    best_epoch = 0
    last_step = (
        ckpt["step"] if opt.load_weights is not None and ckpt.get("step", None) else 0
    )
    best_fitness = (
        ckpt["best_fitness"]
        if opt.load_weights is not None and ckpt.get("best_fitness", None)
        else 0
    )
    ...
    try:
        for epoch in range(opt.num_epochs):
            ...
            for iter, data in enumerate(progress_bar):
                ...
                try:
                    ...
                    (
                        ...
                        anchors,
                        segmentation,
                    ) = model(imgs, annot, seg_annot, obj_list=params.obj_list)
                    cls_loss = (
                        cls_loss.mean() if not opt.freeze_det else torch.tensor(0)
                    )
                    reg_loss = (
                        reg_loss.mean() if not opt.freeze_det else torch.tensor(0)
                    )
                    seg_loss = (
                        seg_loss.mean() if not opt.freeze_seg else torch.tensor(0)
                    )
                    ...
                    if step % opt.save_interval == 0 and step > 0:
                        save_checkpoint(
                            model,
                            opt.saved_path,
                            f"hybridnets-d{opt.compound_coef}_{epoch}_{step}.pth",
                        )
                        print("checkpoint...")
            ...
            if epoch % opt.val_interval == 0:
                best_fitness, best_loss, best_epoch = val(
                    model,
                    val_generator,
                    params,
                    opt,
        ...
        save_checkpoint(
            model, opt.saved_path, f"hybridnets-d{opt.compound_coef}_{epoch}_{step}.pth"
        )
    finally:
        writer.close()
