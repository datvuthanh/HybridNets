...
class AutoDriveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            #     # for label in labels:
            #     #     obj_cls, xmin, ymin, xmax, ymax = label
            #     #     xmin = int(xmin)
            #     #     ymin = int(ymin)
            #     #     xmax = int(xmax)
            #     #     ymax = int(ymax)
            #     #
            #     #     cv2.putText(img_copy,"CLS: " + str(obj_cls),org=(xmin-10,ymin-10), fontFace=font, fontScale=1 ,color=(128,0,0))
            #     #     cv2.rectangle(img_copy,(xmin,ymin), (xmax,ymax), color=(255,0,0), thickness=2)
            #     #
            #     # cv2.imwrite('debug_0.jpg', img_copy)
            #
